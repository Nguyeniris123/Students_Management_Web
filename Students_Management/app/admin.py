...
class StudentView(NhanVienAdminView):
    column_list = ['id', 'name', 'sex', 'birth', 'address', 'phone', 'email', 'class.name', 'user_role']
...
class GradeView(GiaoVienAdminView):
    column_list = ['student.name', 'subject.name', 'semester.year', 'semester.semester_number', 'grade_type', 'grade_value',
        'attempt']
    column_labels = {
...
admin.add_view(GradeView(Score, db.session))  # Chỉ giáo viên có thể truy cập
